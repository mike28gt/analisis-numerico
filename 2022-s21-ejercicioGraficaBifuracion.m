...
% Cantidad de iteraciones para la ecuación logística dentro de los bucles
iteraciones_ecuacion_logistica = 20000;
% Cantidad de valores de x a determinar por cada valor de r
cantidad_datos = 10000;

% Matriz que se utilizará para construir la figura
datos_x = zeros(cantidad_datos, pixeles_r);

% valor inicial de x
x_0 = 0.5;

% ESCRIBA EL MOTOR COMPUTACIONAL DEL CODIGO.
...
histograma_x = zeros(pixeles_x, pixeles_r);
for i=1:pixeles_r
    histograma_x(:, i) = histc(datos_x(:, i), grupos_x);
    histograma_x(:, i) = 255*histograma_x(:, i)/max(histograma_x(:, i));
endfor;
